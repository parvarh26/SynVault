...
export default function Wallets() {
  ...
  const [sweepThreshold, setSweepThreshold] = useState<string>('')
  ...
  const [selectedWallet, setSelectedWallet] = useState<CryptoSymbol>('SEPOLIA_ETH')
  const [wallet1Balance, setWallet1Balance] = useState<number>(cryptoStore.get('SEPOLIA_ETH'))
  ...
  const [sweepExpanded, setSweepExpanded] = useState<boolean>(false)

  const [txs, setTxs] = useState<Transaction[]>([])

  useEffect(() => {
    // Load initial transactions
    setTxs(transactionsStore.getAll())
  ...
  useEffect(() => {
    const unsub = cryptoStore.subscribe((bals) => {
      ...
      <div className="wallet-container" ref={containerRef}>
        ...
        <div className="wallet-grid">
          ...
          <section className="wallet-card">
            ...
            <div className="wallet-subrow" style={{ marginTop: 10, justifyContent: 'space-between', flexWrap: 'wrap' }}>
              <div className="wallet-row" onClick={() => setSweepExpanded(v => !v)} style={{ cursor: 'pointer' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <rect x="4" y="4" width="16" height="16" rx="3" stroke="#a9b2ff" strokeWidth="1.6" />
                  <path d="M7 9h10M7 13h7M7 17h5" stroke="#a9b2ff" strokeWidth="1.6" strokeLinecap="round" />
                </svg>
                <span className="label">Auto Sweep</span>
                <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true" style={{ marginLeft: 8, opacity: .8 }}>
                  <path d="M12 8.8a3.2 3.2 0 1 0 0 6.4 3.2 3.2 0 0 0 0-6.4Zm8 3.2c0-.5-.04-.99-.12-1.46l2.14-1.67-2-3.46-2.54.84c-.74-.62-1.58-1.12-2.5-1.46L14.5 2h-5l-.48 2.12c-.92.34-1.76.84-2.5 1.46l-2.54-.84-2 3.46L4.12 10.5c-.08.47-.12.96-.12 1.46s.04.99.12 1.46l-2.14 1.67 2 3.46 2.54-.84c.74.62 1.58 1.12 2.5 1.46L9.5 22h5l.48-2.12c.92-.34 1.76-.84 2.5-1.46l2.54.84 2-3.46-2.14-1.67c.08-.47.12-.96.12-1.46Z" fill="#9aa0c7"/>
                </svg>
              </div>
              <div className="wallet-sweep-controls">
                <div
                  role="switch"
                  aria-checked={sweepOn}
                  tabIndex={0}
                  className={`wallet-switch ${sweepOn ? 'on' : ''}`}
            ...
            {sweepExpanded && (
              <div className={`wallet-sweep-inline ${sweepOn ? '' : 'disabled'}`} style={{ marginTop: 10 }}>
                <label className="wallet-field-inline">
                  <span>Threshold ({selectedWallet})</span>
                  <input
                    type="number"
                    min="0"
                    step="0.0001"
            ...
            {sweepOn && sweepThreshold && (
              <div className="wallet-sweep-summary" aria-live="polite">
                Auto sweep when balance &gt; <strong>{sweepThreshold}</strong> {selectedWallet}. Excess transfers to Investment automatically.
                {(() => {
                  const th = parseFloat(sweepThreshold)
                  const excess = wallet1Balance - th
                  if (excess > 0) {
                    return (
                      <div style={{ marginTop: 8 }}>
                        <button 
                          ...
                          onClick={() => {
                            ...
                            const amt = Math.round(excess * 1e8) / 1e8
                            const cryptoPrice = prices.get(selectedWallet)
                            const usdValue = amt * cryptoPrice
                            console.log(`Manual Sweep: ${amt} ${selectedWallet} at $${cryptoPrice} = $${usdValue}`)
                            console.log(`Crypto balance before manual sweep: ${cryptoStore.get(selectedWallet)}`)
                            
                            if (wallet1IconRef.current && investIconRef.current) {
                              triggerFly(wallet1IconRef.current, investIconRef.current, 'right', () => {
                              investmentWallet.add(usdValue)
                              cryptoStore.delta(selectedWallet, -amt)
                              
                              console.log(`Crypto balance after manual sweep: ${cryptoStore.get(selectedWallet)}`)
                              const transaction: Transaction = {
                                id: genId(), 
                                date: nowStr(), 
                                coin: selectedWallet, 
                                ...
                                deb: amt, 
                                recipient: 'Investment Wallet', 
                                recipientFull: `Manual sweep ${amt} ${selectedWallet} for $${usdValue.toFixed(2)}`, 
                                self: true, 
                                status: 'Completed',
                                type: 'auto-sweep'
                              }
                              transactionsStore.add(transaction)
                              setTimeout(() => { sweepingRef.current = false }, 50)
                            })
                            }
                          }}
                        >
                          SWEEP NOW ({fmt(excess)} {selectedWallet})
                        </button>
                      </div>
                    )
                  }
            ...
            <div className="wallet-callout">
              <div className="wallet-row" style={{ gap: 12 }}>
                ...
                <div>
                  ...
                  <a className="wallet-link" href="/portfolio">VIEW PORTFOLIO</a>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="wallet-action-body" onClick={(e) => e.stopPropagation()}>
                ...
                <button className="wallet-action-send tip-right" onClick={(e) => {
                  ...
                  // Navigate to investment page (Coming Soon)
                  window.location.href = '/app/investment'
                }}>INVEST</button>
              </div>
            </div>
          </section>
        ...
        <div className="send-table">
          ...
          <div className="send-tbody">
            {txs.length === 0 ? (
              ...
              txs.map(t => (
                <div className="send-trow" key={t.id}>
                  <div>{t.date}</div>
